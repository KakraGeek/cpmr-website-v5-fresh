...
| 2026-05-15 | **E5-S05** (department projects listing) | Winston / programme architect gate | **04 §5–§6**, **05 §4.5**, **05 §5.3–§5.5**, **E6-S02** (canonical `/research/projects/[id]/`) | **PASS** — `/departments/[slug]/projects/` via `getStaticPaths` + `filterDepartmentProjects` on `department_ids`; public slice uses `isPublicDepartment` + `isPublicProject`; card links canonical research detail routes (no duplicate department project detail URLs); static build only; `automation/architect_gates.yml` updated. **Conditions:** (1) optional `/departments/[slug]/projects/[project-slug]/` alias remains deferred — listing + research canonical is sufficient for MVP; (2) follow-up: wire department home “Featured projects” strip to filtered projects (landing still shows pre-E5-S05 empty copy); (3) consider relocating `_project-helpers` to `src/lib/` when a third consumer appears. |
| 2026-05-15 | **E5-S04** (department staff listing) | Winston / programme architect gate | **04 §5–§6**, **05 §4.4**, **05 §5.3–§5.5**, **E6-S03** (canonical `/research/researchers/[id]/`) | **PASS** — `/departments/[slug]/staff/` via `getStaticPaths` + `filterDepartmentStaff` on `department_ids`; `isPublicDepartment` + `isPublicStaff` + `sortPublicStaff`; cards link E6-S03 researcher profiles (no duplicate department staff detail URLs); minimal inline client filters on static HTML; `automation/architect_gates.yml` updated. **Conditions:** (1) wire department home `head_of_department_staff_id` hero block when convenient; (2) amend **05 §5.5** canonical path text to `/research/researchers/`; (3) optional shared department listing-helper extraction when publications/news routes land. |
| 2026-05-15 | **E5-S06** (department publications listing) | Winston / programme architect gate | **04 §5–§6**, **05 §4.6**, **05 §5.5**, **05 §6.1–§6.3**, **E6-S04** (canonical `/research/publications/[id]/`), **ADR-003** | **PASS** — `/departments/[slug]/publications/` via `getStaticPaths` + `filterDepartmentPublications` on `department_ids`; `isPublicDepartment` + `isPublicPublication`; `PublicationItem` rows link E6-S04 publication detail routes (no duplicate department publication URLs); build-time scoping + optional client year filter; PDF surfaces gated via shared `publicationPdfHref`; `automation/architect_gates.yml` updated. **Conditions:** (1) optional `publications-index.json` per **05 §6.2** when build-time filtering cost warrants it; (2) department home publications teaser when featured-publications block is modelled; (3) consider `src/lib/departments/` helper consolidation if news/downloads listings add a fourth consumer. |
